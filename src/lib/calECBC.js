...
export async function getECBCTableSF() {
...
//   0 <= a < ECBC_n
// TODO: get the ECBC table, hash(filename, heading, content...) mod ECBC_n = x => file(x) saved to servers which table[x] = 1
export async function getECBCTableFS() {
  const { ECBC_table } = await createECBCTable();
  let table_fs = [];
...
export async function getServerFileList(files) {
  const table_sf = await getECBCTableSF();
...
class BipGraph {
  ...
  addEdge(u, v) {
    if (this.__match.has(v) === false) this.__match.set(v, this.countN++);
